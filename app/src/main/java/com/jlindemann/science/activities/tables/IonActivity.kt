...
class IonActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        back_btn_ion.setOnClickListener {
            this.onBackPressed()
        }
    }
    ...
    private fun recyclerView() {
        ...
        edit_ion.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable) {
                filter(s.toString(), ionList, recyclerView)
            }
        })
    }
    ...
    private fun filter(text: String, list: ArrayList<Ion>, recyclerView: RecyclerView) {
        ...
        for (item in list) {
            if (item.name.toLowerCase(Locale.ROOT).contains(text.toLowerCase(Locale.ROOT))) { filteredList.add(item) }
